...
@login_required
def buy():
    #"user_id" "name" "shares" "price" "type" "symbol" will be relevant in
...
def buy():
    ...
    if request.method == "POST":
        ...
        #Store the returned dictionary from API
        symbol = request.form.get("symbol")
        api_look = lookup(symbol)

        #Store the shares input
        shares = int(request.form.get("shares"))
        if api_look == None:
            return apology("Please enter a symbol")

        elif not shares.isdigit() or shares < 1:
            return apology("Please assign a positive number")
        ...
        # How much money user_id has
        uid = int(session["user_id"])
        cash = db.execute("SELECT cash FROM users WHERE id = ?", uid)

        # The value of the wanted Purchase
        value = api_look["price"] * int(shares)
        ...
        if cash < value:
            ...
            # Making the sales
            db.execute("UPDATE users SET cash = ? WHERE id = ?", cash - value, uid)

            db.execute("INSERT INTO transactions (user_id, name, shares, price, type, symbol) VALUES(? ,? ,? ,? ,? ,?)",
                       uid, item_name, shares, item_price, 'buy', symbol)
...
def history():
    user_id = session["user_id"]
    transacts = db.execute("SELECT type, symbol, price, shares, time FROM transactions WHERE user_id = ?", user_id)
